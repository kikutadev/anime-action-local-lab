...
- **Run:** the authored sprint remains the source of truth for legs and torso. The sword-arm override now fades to 0% at full locomotion, and the free-arm backward extreme is reduced only during the offending half-stride so the elbow stays near the torso instead of winging behind the back. Run QA now reapplies the same runtime polish every LateUpdate; the earlier one-shot QA path was being overwritten by the Animator and could hide this defect.
